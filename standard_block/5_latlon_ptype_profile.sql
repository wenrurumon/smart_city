...
create table latlon_ptype_profile
as
select t.lat,t.lon,t.ptype,
...
from
(select v.lat, v.lon, u.uid, u.ptype 
from final_grid_id_map v
inner join 
(select s.final_grid_id,s.uid,s.ptype
from openlab.stay_poi s
where s.date = 20170901 and s.city = 'V0110000'
group by s.final_grid_id,s.uid,s.ptype) u
on v.final_grid_id = u.final_grid_id and v.ptype = u.ptype and v.uid = u.uid
) t
